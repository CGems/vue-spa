...
class Loop {
  ...
  listLoop() { // 轮询检查
    ...
    this.timer = setTimeout(() => {
      ...
      first.callback().then(() => { // callback返回后的回调中重置时间、排序轮询队列、执行轮询检查
  ...
  addLoop(name, callback, canFirstExecute) { // 增加一个轮询
    if (!this.config[name]) {
    ...
    this.config[name].previousLoopTimestamp = new Date().getTime() + 11000000000
    this.loopList.indexOf(name) === -1 && this.loopList.push(name) // 已存在只修改配置，不重复加入队列
  ...
  sortLoopList() {
    this.loopList.sort((a, b) => { // 根据执行的时间排个序 近=》远
      return (this.config[a].timeInterval + this.config[a].previousLoopTimestamp) - (this.config[b].timeInterval + this.config[b].previousLoopTimestamp)
    })
  }
}
